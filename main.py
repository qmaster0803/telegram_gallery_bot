...
def photo_handler(message):
    bot.send_message(message.chat.id, "Error! Please send your photos as files to prevent compression.")
...
def doc_handler(message):
    if(storage_module.check_photo_ext(message.document.file_name)):
        if(message.document.file_size > 20971520): #20 MB
            ...
            database_module.add_to_queue(local_filepath, message.chat.id)
...
def process_queue(stop_event):
    while(True):
        ...
        #    print("Queue has candidate(s)!") #TODO
        #    time.sleep(1)
        #else: time.sleep(1)
...
if(__name__ == "__main__"):
    processing_queue_thread_stop_event = threading.Event()
    processing_queue_thread = threading.Thread(target=process_queue, args=(processing_queue_thread_stop_event,))
    processing_queue_thread.start()

    try:bot.infinity_polling()
    except KeyboardInterrupt:
        #wait for sub threads to exit
        processing_queue_thread_stop_event.set()
        #processing_queue_thread.join()
